...
foreign import acsEvent :: Event String
...
scene { w, h } = (drawPoint <$> position <*> keys) where
  background :: Drawing
  ...
  drawPoint {x, y} inSet = case (Set.isEmpty inSet) of
                                true  -> filled (fillColor black) (rectangle x y defaultSize defaultSize)
                                false -> case (Set.member 32 inSet) of
                                              true  -> filled (fillColor black) (circle x y defaultSize)
                                              false -> filled (fillColor white) (circle x y defaultSize)
{-
...
transformAcs str = str <> "foo"

myBehav = transformAcs <$> (step "" acsEvent)


main :: forall eff. Eff (canvas :: CANVAS, frp :: FRP, console :: CONSOLE | eff) Unit
main = do
  ...
  sample_ (scene {w,h}) (interval 20)`subscribe` render ctx
  sample_ (scene {w,h}) move `subscribe` render ctx
  sample_ myBehav (interval 1000) `subscribe` (\x -> log x)
